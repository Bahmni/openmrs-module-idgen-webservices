...
public class IdentifierTypeServiceWrapperImplTest {
	...
	public void shouldGetAllIdentifierType() throws Exception {
		final IdentifierType primaryIdentifierType = new IdentifierType("primary-identifier-type-uuid", "bahmni",
		        "description", ".*", true, true, new ArrayList<IdentifierSource>());
		final IdentifierType extraIdentifierType1 = new IdentifierType("extra-identifier-type-uuid1", "pan", "description",
		        ".*", false, false, new ArrayList<IdentifierSource>());
		final IdentifierType extraIdentifierType2 = new IdentifierType("extra-identifier-type-uuid2", "phone no",
		        "description", ".*", false, false, new ArrayList<IdentifierSource>());
		
		when(Context.getService(IdentifierSourceService.class)).thenReturn(identifierSourceService);
		
		when(Context.getAdministrationService()).thenReturn(administrationService);
		when(administrationService.getGlobalProperty(IdgenWsConstants.GP_PRIMARY_IDTYPE))
		        .thenReturn("primary-identifier-type-uuid");
		when(administrationService.getGlobalProperty(IdgenWsConstants.GP_EXTRA_IDTYPES))
		        .thenReturn("extra-identifier-type-uuid1,extra-identifier-type-uuid2");
		...
		when(patientService.getPatientIdentifierTypeByUuid("primary-identifier-type-uuid"))
		        .thenReturn(primaryPatientIdentifierType);
		when(patientService.getPatientIdentifierTypeByUuid("extra-identifier-type-uuid1"))
		        .thenReturn(extraPatientIdentifierType1);
		when(patientService.getPatientIdentifierTypeByUuid("extra-identifier-type-uuid2"))
		        .thenReturn(extraPatientIdentifierType2);
